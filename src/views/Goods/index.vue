...
<template>
  <div class="goods-index supplies-set">
    ...
    <div class="con">
        <div class="l l1">
            ...
            <div class="list-box">
                <ul class="list" id="list1">
                    <li v-for="(item ,index) in goodsList" :key = "index" :class="item.id == selectGoods.id? 'active':''" @click="selectGoods = item">
                      <span class="text">{{item.name}}</span>
                      <el-icon @click.stop="setEditGoods(item, 1)" class="icon-btn is_hide"><Edit /></el-icon>
                      <el-icon @click.stop="delGoodsData(item.id)" class="icon-btn is_hide"><Delete /></el-icon>
                    </li>
                </ul>
            </div>
        </div>
        <div class="l l2">
            <div class="top-bar">
              <el-button v-if="selectGoods.id" size="small" type="primary" icon="Plus" @click="setAddGoods(2)">位置/版面</el-button>
            </div>
            <div class="list-box">
                <ul class="list" id="list2">
                    <li v-for="(item ,index) in goodsList2" :key="index" :class="item.id == selectGoods2.id? 'active':''" @click="selectGoods2 = item">
                        <span class="text">{{item.name}}</span>
                        <el-icon @click.stop="setEditGoods(item, 2)" class="icon-btn is_hide"><Edit /></el-icon>
                        <el-icon @click.stop="delGoodsData(item.id)" class="icon-btn is_hide"><Delete /></el-icon>
                    </li>
                </ul>
            </div>
        ...
        <div class="l l3">
            <div class="top-bar">
              <el-button v-if="selectGoods2.id" size="small" type="primary" icon="Plus" @click="setAddGoods(3)">规格/尺寸</el-button>
            </div>
            <div class="list-box" style="padding: 10px;">
                <ul class="list list2" id="list3">
                    <li v-for="(item ,index) in goodsList3" :key="index" :class="item.id == selectGoods3.id? 'active':''" @click="selectGoods3 = item">
                      <span class="text">{{item.name}}</span>
                      <el-icon @click.stop="delGoodsData(item.id)" class="icon-btn"><Close /></el-icon>
                    </li>
                </ul>

                <el-form v-if="selectGoods3.id" ref="settingFormRef" :model="settingData" style="margin-top: 20px" label-width="auto">
                  <el-form-item label="单价" prop="price" :rules="[{ required: true, message: '请输入', trigger: 'blur' }, ..._rules.positive]">
                  ...
                  <el-form-item label="上架数量" prop="num" :rules="[{ required: true, message: '请输入', trigger: 'blur' }, ..._rules.positive]">
              ...
              <el-table ref="tableRef" :data="goodsListSetting" border>
                ...
                <el-table-column label="操作" fixed="right" width="170">
                  <template #default="scope">
                        ...
                        <el-button link type="primary" @click="changeEnable(1,0,scope.row.id)" v-if="scope.row.isEnable == 1">已开启</el-button>
