    ...
    <el-tab-pane label="销售客户">
      ...
      >
        ...
        <el-table-column prop="type" label="类型">
          <template #default="scope: any">
            {{ (() =>{ 
                let d = types.find((item:any) => item.value === scope.row.type)
                return d? d.label : ''
              })()
            }}
          </template>
        ...
        <el-table-column prop="isRequired" label="必填">
          <template #default="scope: any">
            <el-checkbox v-model="scope.row.isRequired" :true-label="1" :false-label="0" @change="changeRequired(scope.row, scope.$index)"></el-checkbox>
          </template>
        </el-table-column>
        <el-table-column prop="isEnable" label="启用">
          <template #default="scope">
            <el-checkbox v-model="scope.row.isEnable" :true-label="1" :false-label="0" @change="changeEnable(scope.row, scope.$index)"></el-checkbox>
          </template>
        </el-table-column>
