    ...
    <div class="s-flex-auto" style="min-height: 0;">
      <el-table ref="tableRef" :data="tableData" border table-layout="fixed" 
        ...
        <el-table-column fixed="right" label="操作" width="220">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="goodsDetail(scope.row.id)">详情</el-button>
            <template v-if="scope.row.orderStatus !== 0">
              <template v-if="scope.row.contractStatus === 0">
                <el-button link type="primary" size="small" @click="$router.push({ name: 'ContractTemplates', query: { id: scope.row.id, type: 2 } })">签订合同</el-button>
                <el-button link type="primary" size="small" @click="setContract(scope.row.id)">上传合同</el-button>
              </template>
              <template v-else>
                <el-button link type="primary" size="small" @click="$router.push({ name: 'ContractTemplates', query: { id: scope.row.id, type: 2 } })">更新合同</el-button>
                <el-button link type="primary" size="small" @click="setContract(scope.row.id)">上传合同</el-button>
              </template>
